...
- Create a bucket of `latif.source.lambda` with following properties,

```text
Bucket name                 : latif.source.lambda
Region                      : N.Virginia
...
- Create another bucket of `latif.destination.lambda` with following properties,

```text
Bucket name                 : latif.destination.lambda
Region                      : N.Virginia
Block all public access     : Checked (KEEP BlOCKED)
Versioning                  : Disabled
Tagging                     : 0 Tags
...
def lambda_handler(event, context):
   destination_bucket_name = 'latif.destination.lambda'
...
- Select S3 bucket named 'latif.source.lambda'

- Upload any files to source bucket

- Go to the S3 bucket named 'latif.destination.lambda' and show the uploaded files to the source bucket.
